...
def _(mo, repos_table):
    ...
    delete_repo_btn = mo.ui.run_button(
        label=f"Delete ({_count})" if _has_selection else "Delete Selected",
        kind="danger",
        disabled=not _has_selection
    )
    mo.hstack([
        delete_repo_btn,
        mo.md(f"Selected: **{_names}**" if _has_selection else "_Select repositories to delete_")
    ])
...
def _(get_ft_refresh, mo, session):
    ...
    _file_types = session.get_file_types()
    _rows = [
        {"Extension": ft["extension"], "Type": ft["file_type"], "Subtype": ft.get("subtype", "") or ""}
        for ft in _file_types[:50]
    ]
...
def _(ft_ext_display, ft_save_btn, ft_subtype_input, ft_type_input, get_ft_refresh, mo, session, set_ft_refresh):
    if ft_save_btn and ft_save_btn.value and ft_ext_display:
        ...
        _ok = session.update_file_type(
            ft_ext_display.value,
            ft_type_input.value,
            ft_subtype_input.value
        )
...
def _(get_ext_refresh, mo, session):
    ...
    ext_node_type_select = mo.ui.dropdown(
        options=_options,
        label="Node Type",
    ...
    if ext_node_type_select and ext_node_type_select.value:
        ...
        if _cfg:
            ...
            ext_instruction = mo.ui.text_area(
                value=_cfg["instruction"] or "", label="Instruction", rows=3
            )
            ext_input_sources = mo.ui.text(
                value=_cfg["input_sources"] or "", label="Input Sources"
            )
...
def _(get_der_refresh, mo, session):
    ...
    der_element_type_select = mo.ui.dropdown(
        options=_options,
        label="Element Type",
        value=_options[0] if _options else None
    )
