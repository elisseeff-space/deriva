...
class TestIsLikelyDataObject:
    ...
    def test_no_match_returns_false(self):
        ...
        assert _is_likely_data_object("random_func", {"model"}, set()) is False
...
class TestFilterCandidates:
    ...
    def test_filters_empty_names(self):
        ...
        candidates = [make_candidate(""), make_candidate("UserModel")]
        result = filter_candidates(candidates, {}, {"model"}, set(), 10)
    ...
    def test_fills_remaining_slots(self):
        ...
        likely = [make_candidate(f"Model_{i}", pagerank=0.9) for i in range(2)]
        others = [make_candidate(f"other_{i}", pagerank=0.5) for i in range(10)]
        result = filter_candidates(likely + others, {}, {"model"}, set(), 8)
        assert len(result) <= 8
...
class TestGenerate:
    ...
    @patch("deriva.modules.derivation.data_object.get_enrichments")
    @patch("deriva.modules.derivation.data_object.query_candidates")
    @patch("deriva.modules.derivation.data_object.filter_candidates")
    ...
    @patch("deriva.modules.derivation.data_object.query_candidates")
    @patch("deriva.modules.derivation.data_object.filter_candidates")
    @patch("deriva.modules.derivation.data_object.batch_candidates")
    def test_handles_llm_error(self, mock_batch, mock_filter, mock_query, mock_enrich, mock_config):
        from deriva.modules.derivation.data_object import generate

        mock_query.return_value = [make_candidate("model")]
        mock_filter.return_value = [make_candidate("model")]
        mock_batch.return_value = [[make_candidate("model")]]
        mock_enrich.return_value = {}
        mock_config.get_derivation_patterns.return_value = {"include": set(), "exclude": set()}
