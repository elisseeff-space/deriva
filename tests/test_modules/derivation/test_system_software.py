...
class TestIsLikelySystemSoftware:
    ...
    def test_exclude_pattern_takes_precedence(self):
        ...
        assert _is_likely_system_software("test_daemon", {"daemon"}, {"test"}) is False

    def test_no_match_returns_false(self):
        from deriva.modules.derivation.system_software import _is_likely_system_software

        assert _is_likely_system_software("random_func", {"daemon"}, set()) is False
...
class TestFilterCandidates:
    ...
    def test_filters_empty_names(self):
        ...
        candidates = [make_candidate(""), make_candidate("worker_daemon")]
        result = filter_candidates(candidates, {}, {"daemon"}, set(), 10)
    ...
    def test_fills_remaining_slots(self):
        ...
        likely = [make_candidate(f"daemon_{i}", pagerank=0.9) for i in range(2)]
        others = [make_candidate(f"other_{i}", pagerank=0.5) for i in range(10)]
        result = filter_candidates(likely + others, {}, {"daemon"}, set(), 8)
        assert len(result) <= 8
...
class TestGenerate:
    ...
    @patch("deriva.modules.derivation.system_software.get_enrichments")
    @patch("deriva.modules.derivation.system_software.query_candidates")
    @patch("deriva.modules.derivation.system_software.filter_candidates")
    ...
    @patch("deriva.modules.derivation.system_software.query_candidates")
    @patch("deriva.modules.derivation.system_software.filter_candidates")
    @patch("deriva.modules.derivation.system_software.batch_candidates")
    def test_handles_llm_error(self, mock_batch, mock_filter, mock_query, mock_enrich, mock_config):
        from deriva.modules.derivation.system_software import generate

        mock_query.return_value = [make_candidate("daemon")]
        mock_filter.return_value = [make_candidate("daemon")]
        mock_batch.return_value = [[make_candidate("daemon")]]
        mock_enrich.return_value = {}
        mock_config.get_derivation_patterns.return_value = {"include": set(), "exclude": set()}
